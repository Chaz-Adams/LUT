...
let isOpen = function (drawer) {
  ...
  openDrawer = function (drawer) {
    if (!isOpen(drawer)) {
      if (!anyOpenDrawers()) {
        let p = drawer.parent();
        if (p.hasClass('drawer-push') || p.hasClass('drawer-slide')) p.addClass('open');
  ...
  closeDrawer = function (e, drawer) {
    if (typeof drawer === 'undefined') {
      ...
      if (drawer.length === 0) drawer = $('.navbar.fixed-top.off-canvas[data-open-drawer="1"]');
    }
  ...
  getRightDrawer = function () {
    ...
    if (d.length === 0) d = $('.navbar.fixed-top.off-canvas[data-right-drawer="1"]');
  ...
  toggleDrawer = function (drawer) {
    if (isOpen(drawer)) closeDrawer(drawer);
