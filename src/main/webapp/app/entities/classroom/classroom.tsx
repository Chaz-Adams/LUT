          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('classroomId')}>
                  Classroom Id <FontAwesomeIcon icon={getSortIconByFieldName('classroomId')} />
                </th>
                <th className="hand" onClick={sort('className')}>
                  Class Name <FontAwesomeIcon icon={getSortIconByFieldName('className')} />
                </th>
                <th>
                  Students <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {classroomList.map((classroom, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {classroom.students
                      ? classroom.students.map((val, j) => (
                          <span key={j}>
                            <Link to={`/student/${val.id}`}>{val.id}</Link>
                            {j === classroom.students.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/classroom/${classroom.id}/delete`)}
                        color="danger"
                        size="sm"
